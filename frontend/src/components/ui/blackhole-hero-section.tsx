...
export function BlackHoleHeroSection({
  ...
  useEffect(() => {
    ...
    if (isGL2) {
      ...
      if (ok) {
        ...
        internal = (g2 as any).RGBA16F || gl.RGBA;
      } else hdr = false;
    } else {
      const hf = gl.getExtension("OES_texture_half_float");
      const cb = gl.getExtension("EXT_color_buffer_half_float");
      if (hf && cb) {
        texType = (hf as any).HALF_FLOAT_OES;
        internal = gl.RGBA;
      } else hdr = false;
    }
    if (!hdr) {
  ...
  return (
    ...
    >
      ...
      {children ? <div className="relative z-10 h-full min-h-screen w-full flex flex-col">{children}</div> : null}
    </div>
  );
}
